        ...
        .map((products, index) => {
          return /*html */ `
                    <tr>
                        <td>${index}</td>
                        <td>${products.name}</td>
                        <td><img width='70px' src="${products.image}"></td>
                        <td>
                        ${products.categoriesID}
                        
                        </td>
                        <td class="ds-td">${products.description}</td>
                        <td>${new Intl.NumberFormat('ja-JP').format(products.price)}</td>

                        <td>${products.quantity}</td>
                     
                        <td>
                        <a href="/#/admineditproduct/${
                          products.id
                        }"> <button style="border-radius: 6px;" type="button" class="btn btn-secondary"><i class="far fa-edit" ></i></button></a>
                        <button style="border-radius: 6px;" data-id="${
                          products.id
                        }" type="button" class="btn btn-danger btn-remove"> <i class="fas fa-trash-alt"></i></button>
                        </td>
                    </tr>
      ...
      btn.addEventListener("click", async function () {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            await reRender(ProductAdmin, "#root");
          }
        });
      });
    });
